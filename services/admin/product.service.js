...
export class ProductService {
  ...
  renderList() {
    ...
    this.productList.forEach((p, idx) => {
      ...
      tr.innerHTML = `
                ...
                }" class="rounded" style="width:50px;height:50px;object-fit:cover;" /></td>
                <td>
                    <a class="btn btn-sm btn-sm btn-outline-primary me-1" href="product_edit.html?id=${encodeURIComponent(
                      p.id || ""
                    )}" title="Sửa"><i class="bi bi-pencil"></i></a>
                    <button class="btn btn-sm btn-outline-danger btn-delete" data-id="${
                      p.id || ""
                    }" data-bs-toggle="modal" data-bs-target="#deleteModal" title="Xóa"><i class="bi bi-trash"></i></button>
                </td>
            `;
  ...
  setupConfirmDelete() {
    ...
    document
      .querySelector("#productTableBody")
      ...
      .then((res) => {
        if (res.status == status.CREATED || res.status == status.OK) {
          ...
          if (addModalEl2 && typeof bootstrap !== "undefined") {
            const modalInstance =
              bootstrap.Modal.getInstance(addModalEl2) ||
              new bootstrap.Modal(addModalEl2);
            modalInstance.hide();
          }
        } else {
