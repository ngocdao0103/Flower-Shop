...
export class BlogService {
                            ...
                            <div class="modal-content border-0 shadow">
                                <div class="modal-body text-center p-4">
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <button onclick="deleteBlog('${blog.id}')" class="btn btn-danger ms-2">
                                            Xóa
                                        </button>
                                    </div>
                                </div>
  ...
  async createBlog(title, slug, content, thumbnail_url, author, category) {
    ...
    if (res.status === status.CREATED || res.status === status.OK) {
      sessionStorage.setItem("delete_blog_success", "Thêm thành công");
  ...
  async deleteBlog(id) {
    ...
    if (res.status === status.OK) {
      sessionStorage.setItem("delete_blog_success", "Xóa thành công");
      sessionStorage.setItem("classMessage", "success");
      await this.renderBlogList();
    } else {
